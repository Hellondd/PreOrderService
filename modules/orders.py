...
class OrderModule:
    ...
    def create_preorder(user_id, sku, qty):
        ...
        if available >= qty:
            status = "Pending"
            msg = f"Товар {sku} ({qty} шт.) забронирован. Он уже находится в пути на склад."
        else:
            status = "Waitlist"
            msg = f"Товар {sku} ({qty} шт.) добавлен в лист ожидания. Как только появится поставка, он будет забронирован автоматически."
